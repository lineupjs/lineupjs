...
import { setText, adaptDynamicColorToBgColor, noRenderer, BIG_MARK_SATURATION_FACTOR, adaptColor } from './utils';
  ...
  create(col: INumberColumn, context: IRenderContext, imposer?: IImposer): ICellRenderer {
    ...
    return {
      ...
      update: (n: HTMLDivElement, d: IDataRow) => {
        ...
        const color = adaptColor(colorOf(col, d, imposer, value), BIG_MARK_SATURATION_FACTOR);
      ...
      render: (ctx: CanvasRenderingContext2D, d: IDataRow) => {
        ...
        const value = col.getNumber(d);
        ctx.fillStyle = adaptColor(colorOf(col, d, imposer, value) || DEFAULT_COLOR, BIG_MARK_SATURATION_FACTOR);
        const w = width * value;
